...
event,0===event.toLowerCase().indexOf(inputValue.toLowerCase())&&0<inputValue.length&&inputRef.current.setSelectionRange(inputValue.length,event.length))))}),checkHighlightedOptionExists=()=>{const isSameValue=(value1,value2)=>{value1=value1?getOptionLabel(value1):"";value2=value2?getOptionLabel(value2):"";return value1===value2};if(-1!==highlightedIndexRef.current&&previousProps.filteredOptions&&previousProps.filteredOptions.length!==filteredOptions.length&&(multiple?value$jscomp$0.length===previousProps.value.length&&
previousProps.value.every((val,i)=>getOptionLabel(value$jscomp$0[i])===getOptionLabel(val)):isSameValue(previousProps.value,value$jscomp$0))){const previousHighlightedOption=previousProps.filteredOptions[highlightedIndexRef.current];if(previousHighlightedOption&&filteredOptions.some(option=>getOptionLabel(option)===getOptionLabel(previousHighlightedOption)))return!0}return!1},syncHighlightedIndex=React.useCallback(()=>{if(popupOpen&&!checkHighlightedOptionExists()){var valueItem=multiple?value$jscomp$0[0]:
value$jscomp$0;if(0===filteredOptions.length||null==valueItem)changeHighlightedIndex({diff:"reset"});else if(listboxRef.current)if(null!=valueItem){const currentOption=filteredOptions[highlightedIndexRef.current];if(!multiple||!currentOption||-1===findIndex(value$jscomp$0,val=>isOptionEqualToValue(currentOption,val))){var itemIndex=findIndex(filteredOptions,optionItem=>isOptionEqualToValue(optionItem,valueItem));-1===itemIndex?changeHighlightedIndex({diff:"reset"}):setHighlightedIndex({index:itemIndex})}}else highlightedIndexRef.current>=
filteredOptions.length-1?setHighlightedIndex({index:filteredOptions.length-1}):setHighlightedIndex({index:highlightedIndexRef.current})}},[filteredOptions.length,multiple?!1:value$jscomp$0,filterSelectedOptions,changeHighlightedIndex,setHighlightedIndex,popupOpen,inputValue,multiple]),handleListboxRef=(0,_utils.unstable_useEventCallback)(node=>{(0,_utils.unstable_setRef)(listboxRef,node);node&&syncHighlightedIndex()});React.useEffect(()=>{inputRef.current&&"INPUT"===inputRef.current.nodeName||(inputRef.current&&
"TEXTAREA"===inputRef.current.nodeName?console.warn([`A textarea element was provided to ${componentName} where input was expected.`,"This is not a supported scenario but it may work under certain conditions.\nA textarea keyboard navigation may conflict with Autocomplete controls (e.g. enter and arrow keys).\nMake sure to test keyboard navigation and add custom event handlers if necessary."].join("\n")):console.error([`MUI: Unable to find the input element. It was resolved to ${inputRef.current} while an HTMLInputElement was expected.`,
`Instead, ${componentName} expects an input element.`,"","useAutocomplete"===componentName?"Make sure you have bound getInputProps correctly and that the normal ref/effect resolutions order is guaranteed.":"Make sure you have customized the input component correctly."].join("\n")))},[componentName]);React.useEffect(()=>{syncHighlightedIndex()},[syncHighlightedIndex]);const handleOpen=event=>{open||(setOpenState(!0),setInputPristine(!0),onOpen&&onOpen(event))},handleClose=(event,reason)=>{open&&(setOpenState(!1),
onClose&&onClose(event,reason))},handleValue=(event,newValue,reason,details)=>{if(multiple){if(value$jscomp$0.length===newValue.length&&value$jscomp$0.every((val,i)=>val===newValue[i]))return}else if(value$jscomp$0===newValue)return;onChange&&onChange(event,newValue,reason,details);setValueState(newValue)},isTouch=React.useRef(!1),selectNewValue=(event,option,reasonProp="selectOption",origin="options")=>{let newValue=option;if(multiple){newValue=Array.isArray(value$jscomp$0)?value$jscomp$0.slice():
[];var matches=newValue.filter(val=>isOptionEqualToValue(option,val));1<matches.length&&console.error([`MUI: The \`isOptionEqualToValue\` method of ${componentName} does not handle the arguments correctly.`,`The component expects a single value to match a given option but found ${matches.length} matches.`].join("\n"));matches=findIndex(newValue,valueItem=>isOptionEqualToValue(option,valueItem));-1===matches?newValue.push(option):"freeSolo"!==origin&&(newValue.splice(matches,1),reasonProp="removeOption")}resetInputValue(event,
newValue);handleValue(event,newValue,reasonProp,{option});disableCloseOnSelect||event&&(event.ctrlKey||event.metaKey)||handleClose(event,reasonProp);(!0===blurOnSelect||"touch"===blurOnSelect&&isTouch.current||"mouse"===blurOnSelect&&!isTouch.current)&&inputRef.current.blur()},handleFocusTag=(event,direction)=>{if(multiple){""===inputValue&&handleClose(event,"toggleInput");event=focusedTag;-1===focusedTag?""===inputValue&&"previous"===direction&&(event=value$jscomp$0.length-1):(event+="next"===direction?
1:-1,0>event&&(event=0),event===value$jscomp$0.length&&(event=-1));a:if(-1===event)event=-1;else for(;;){if("next"===direction&&event===value$jscomp$0.length||"previous"===direction&&-1===event){event=-1;break a}const option=anchorEl.querySelector(`[data-tag-index="${event}"]`);if(option&&option.hasAttribute("tabindex")&&!option.disabled&&"true"!==option.getAttribute("aria-disabled"))break a;else event+="next"===direction?1:-1}setFocusedTag(event);focusTag(event)}},handleClear=event=>{ignoreFocus.current=
!0;setInputValueState("");onInputChange&&onInputChange(event,"","clear");handleValue(event,multiple?[]:null,"clear")},handleKeyDown=other=>event=>{if(other.onKeyDown)other.onKeyDown(event);if(!event.defaultMuiPrevented&&(-1!==focusedTag&&-1===["ArrowLeft","ArrowRight"].indexOf(event.key)&&(setFocusedTag(-1),focusTag(-1)),229!==event.which))switch(event.key){case "Home":popupOpen&&handleHomeEndKeys&&(event.preventDefault(),changeHighlightedIndex({diff:"start",direction:"next",reason:"keyboard",event}));
break;case "End":popupOpen&&handleHomeEndKeys&&(event.preventDefault(),changeHighlightedIndex({diff:"end",direction:"previous",reason:"keyboard",event}));break;case "PageUp":event.preventDefault();changeHighlightedIndex({diff:-5,direction:"previous",reason:"keyboard",event});handleOpen(event);break;case "PageDown":event.preventDefault();changeHighlightedIndex({diff:5,direction:"next",reason:"keyboard",event});handleOpen(event);break;case "ArrowDown":event.preventDefault();changeHighlightedIndex({diff:1,
direction:"next",reason:"keyboard",event});handleOpen(event);break;case "ArrowUp":event.preventDefault();changeHighlightedIndex({diff:-1,direction:"previous",reason:"keyboard",event});handleOpen(event);break;case "ArrowLeft":handleFocusTag(event,"previous");break;case "ArrowRight":handleFocusTag(event,"next");break;case "Enter":if(-1!==highlightedIndexRef.current&&popupOpen){var option=filteredOptions[highlightedIndexRef.current],disabled=getOptionDisabled?getOptionDisabled(option):!1;event.preventDefault();
disabled||(selectNewValue(event,option,"selectOption"),autoComplete&&inputRef.current.setSelectionRange(inputRef.current.value.length,inputRef.current.value.length))}else freeSolo&&""!==inputValue&&!1===inputValueIsSelectedValue&&(multiple&&event.preventDefault(),selectNewValue(event,inputValue,"createOption","freeSolo"));break;case "Escape":popupOpen?(event.preventDefault(),event.stopPropagation(),handleClose(event,"escape")):clearOnEscape&&(""!==inputValue||multiple&&0<value$jscomp$0.length)&&(event.preventDefault(),
event.stopPropagation(),handleClear(event));break;case "Backspace":multiple&&!readOnly&&""===inputValue&&0<value$jscomp$0.length&&(option=-1===focusedTag?value$jscomp$0.length-1:focusedTag,disabled=value$jscomp$0.slice(),disabled.splice(option,1),handleValue(event,disabled,"removeOption",{option:value$jscomp$0[option]}));break;case "Delete":multiple&&!readOnly&&""===inputValue&&0<value$jscomp$0.length&&-1!==focusedTag&&(option=value$jscomp$0.slice(),option.splice(focusedTag,1),handleValue(event,option,
"removeOption",{option:value$jscomp$0[focusedTag]}))}},handleFocus=event=>{setFocused(!0);openOnFocus&&!ignoreFocus.current&&handleOpen(event)},handleBlur=event=>{unstable_isActiveElementInListbox(listboxRef)?inputRef.current.focus():(setFocused(!1),firstFocus.current=!0,ignoreFocus.current=!1,autoSelect&&-1!==highlightedIndexRef.current&&popupOpen?selectNewValue(event,filteredOptions[highlightedIndexRef.current],"blur"):autoSelect&&freeSolo&&""!==inputValue?selectNewValue(event,inputValue,"blur",
"freeSolo"):clearOnBlur&&resetInputValue(event,value$jscomp$0),handleClose(event,"blur"))},handleInputChange=event=>{const newValue=event.target.value;inputValue!==newValue&&(setInputValueState(newValue),setInputPristine(!1),onInputChange&&onInputChange(event,newValue,"input"));""===newValue?disableClearable||multiple||handleValue(event,null,"clear"):handleOpen(event)},handleOptionMouseOver=event=>{setHighlightedIndex({event,index:Number(event.currentTarget.getAttribute("data-option-index")),reason:"mouse"})},
handleOptionTouchStart=()=>{isTouch.current=!0},handleOptionClick=event=>{const index=Number(event.currentTarget.getAttribute("data-option-index"));selectNewValue(event,filteredOptions[index],"selectOption");isTouch.current=!1},handleTagDelete=index=>event=>{const newValue=value$jscomp$0.slice();newValue.splice(index,1);handleValue(event,newValue,"removeOption",{option:value$jscomp$0[index]})},handlePopupIndicator=event=>{open?handleClose(event,"toggleInput"):handleOpen(event)},handleMouseDown=event=>
{event.target.getAttribute("id")!==id&&event.preventDefault()},handleClick=()=>{inputRef.current.focus();selectOnFocus&&firstFocus.current&&0===inputRef.current.selectionEnd-inputRef.current.selectionStart&&inputRef.current.select();firstFocus.current=!1},handleInputMouseDown=event=>{""!==inputValue&&open||handlePopupIndicator(event)};props=(props=freeSolo&&0<inputValue.length)||(multiple?0<value$jscomp$0.length:null!==value$jscomp$0);let groupedOptions=filteredOptions;if(groupBy){const indexBy=new Map;
let warn=!1;groupedOptions=filteredOptions.reduce((acc,option,index)=>{const group=groupBy(option);0<acc.length&&acc[acc.length-1].group===group?acc[acc.length-1].options.push(option):(indexBy.get(group)&&!warn&&(console.warn(`MUI: The options provided combined with the \`groupBy\` method of ${componentName} returns duplicated headers.`,"You can solve the issue by sorting the options with the output of `groupBy`."),warn=!0),indexBy.set(group,!0),acc.push({key:index,index,group,options:[option]}));
return acc},[])}disabledProp&&focused&&handleBlur();return{getRootProps:(other={})=>(0,_extends2.default)({"aria-owns":listboxAvailable?`${id}-listbox`:null},other,{onKeyDown:handleKeyDown(other),onMouseDown:handleMouseDown,onClick:handleClick}),getInputLabelProps:()=>({id:`${id}-label`,htmlFor:id}),getInputProps:()=>({id,value:inputValue,onBlur:handleBlur,onFocus:handleFocus,onChange:handleInputChange,onMouseDown:handleInputMouseDown,"aria-activedescendant":popupOpen?"":null,"aria-autocomplete":autoComplete?
"both":"list","aria-controls":listboxAvailable?`${id}-listbox`:void 0,"aria-expanded":listboxAvailable,autoComplete:"off",ref:inputRef,autoCapitalize:"none",spellCheck:"false",role:"combobox",disabled:disabledProp}),getClearProps:()=>({tabIndex:-1,onClick:handleClear}),getPopupIndicatorProps:()=>({tabIndex:-1,onClick:handlePopupIndicator}),getTagProps:({index})=>(0,_extends2.default)({key:index,"data-tag-index":index,tabIndex:-1},!readOnly&&{onDelete:handleTagDelete(index)}),getListboxProps:()=>({role:"listbox",
id:`${id}-listbox`,"aria-labelledby":`${id}-label`,ref:handleListboxRef,onMouseDown:event=>{event.preventDefault()}}),getOptionProps:({index,option})=>{const selected=(multiple?value$jscomp$0:[value$jscomp$0]).some(value2=>null!=value2&&isOptionEqualToValue(option,value2)),disabled=getOptionDisabled?getOptionDisabled(option):!1;return{key:getOptionLabel(option),tabIndex:-1,role:"option",id:`${id}-option-${index}`,onMouseOver:handleOptionMouseOver,onClick:handleOptionClick,onTouchStart:handleOptionTouchStart,
