...
shadow$provide.module$node_modules$$mui$base$node$useListbox$defaultListboxReducer=function(global,require,module,exports){function findValidOptionToHighlight(index,lookupDirection,options,focusDisabled,isOptionDisabled,wrapAround){if(0===options.length||options.every((o,i)=>isOptionDisabled(o,i)))return-1;for(;;){if(!wrapAround&&"next"===lookupDirection&&index===options.length||!wrapAround&&"previous"===lookupDirection&&-1===index)return-1;if(focusDisabled?0:isOptionDisabled(options[index],index))index+=
"next"===lookupDirection?1:-1,wrapAround&&(index=(index+options.length)%options.length);else return index}}function getNewHighlightedOption(options,previouslyHighlightedOption,diff,lookupDirection,highlightDisabled,isOptionDisabled,wrapAround,optionComparer){var _options$nextIndex;const maxIndex=options.length-1,previouslyHighlightedIndex=null==previouslyHighlightedOption?-1:options.findIndex(option=>optionComparer(option,previouslyHighlightedOption));if("reset"===diff)return null;if("start"===diff)diff=
0;else if("end"===diff)diff=maxIndex;else{const newIndex=previouslyHighlightedIndex+diff;diff=0>newIndex?!wrapAround&&-1!==previouslyHighlightedIndex||1<Math.abs(diff)?0:maxIndex:newIndex>maxIndex?!wrapAround||1<Math.abs(diff)?maxIndex:0:newIndex}lookupDirection=findValidOptionToHighlight(diff,lookupDirection,options,highlightDisabled,isOptionDisabled,wrapAround);return null!=(_options$nextIndex=options[lookupDirection])?_options$nextIndex:null}function handleOptionSelection(option,state,props){const {multiple,
optionComparer=(o,v)=>o===v,isOptionDisabled=()=>!1}=props,{selectedValue}=state,optionIndex=props.options.findIndex(o=>props.optionComparer(option,o));return isOptionDisabled(option,optionIndex)?state:multiple?{selectedValue:(null!=selectedValue?selectedValue:[]).some(sv=>optionComparer(sv,option))?selectedValue.filter(v=>!optionComparer(v,option)):[...(null!=selectedValue?selectedValue:[]),option],highlightedValue:option}:null!=selectedValue&&optionComparer(option,selectedValue)?state:{selectedValue:option,
highlightedValue:option}}function handleKeyDown(event,state,props){const {options,isOptionDisabled,disableListWrap,disabledItemsFocusable,optionComparer}=props,moveHighlight=(diff,direction,wrapAround)=>getNewHighlightedOption(options,state.highlightedValue,diff,direction,null!=disabledItemsFocusable?disabledItemsFocusable:!1,null!=isOptionDisabled?isOptionDisabled:()=>!1,wrapAround,optionComparer);switch(event.key){case "Home":return(0,_extends2.default)({},state,{highlightedValue:moveHighlight("start",
...
//# sourceMappingURL=module$node_modules$$mui$base$node$useListbox$defaultListboxReducer.js.map
